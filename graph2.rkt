...
# Test for DFS on a fully disconnected graph
test 'dfs_fully_disconnected_graph':
    let g = WUGraph(5)  
    assert dfs_to_list(g, 0) == Cons.from_vec([0])
    assert dfs_to_list(g, 1) == Cons.from_vec([1])
    assert dfs_to_list(g, 2) == Cons.from_vec([2])
    assert dfs_to_list(g, 3) == Cons.from_vec([3])
    assert dfs_to_list(g, 4) == Cons.from_vec([4])
