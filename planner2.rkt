...
def dijkstraPQ(graph: WUGraph?, start) -> TupC[vec?,vec?]:
    ...
    dist[start] = 0
    let todo = BinHeap[PQ?](Cons.len(graph.get_all_edges()), lambda x, y: x.priority <= y.priority)
...
test 'plan_route on complex tp':
    let tp = TripPlanner(
    [[0,0,0,1],[0,1,3,0],[0,1,4,0],[0,1,5,0],[0,1,6,0],[0,0,1,1],[1,1,3,0],[1,1,4,0],
    [1,1,5,0],[1,1,6,0],[0,0,2,1],[2,1,3,0],[2,1,4,0],[2,1,5,0],[2,1,6,0]],
    [[0,0,'blacksmith',"Rever's Silver Shop"],
